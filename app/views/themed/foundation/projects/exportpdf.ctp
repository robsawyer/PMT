<?php 
	...
	$tcpdf->xheadertext = 'Ad Creative Project Development Report - '.date('m/d/Y'); 
	...
	$html .= 'Offshore projects: <span class="number-count">'.count($offshoreProjects).'</span> | ';
	$html .= 'In-house only projects: <span class="number-count">'.count($inhouseProjects).'</span> | ';
	...
	foreach($updatedProjects as $project){
		...
		$html .= "<li>Offshore: ".$project['Project']['offshore']."</li>";
	...
	echo $tcpdf->Output('ad_creative_report_'.date('Y-m-d').'.pdf', 'D'); 
?>